 ...
 public:
    ...
    CACHE_TAG Replace(CACHE_TAG tag)
    {
        ...
        if (_tags.size() > _associativity) {
            int index = my_random_number % _associativity;
            ret = _tags[index];
...
UINT32 TWO_LEVEL_CACHE<SET>::Access(ADDRINT addr, ACCESS_TYPE accessType)
{
    ...
    if (!l1Hit) {
        ...
        if (!l2Hit) {
            ...
            cout << "Is it time for Prefetching?"<< endl;
            if ((L2_INCLUSIVE == 1) && !(l2_replaced == INVALID_TAG)) {
            	cout << "Yep!! N = " << _l2_prefetch_lines << endl;
	        ...
	        for (UINT32 i=0; i < _l2_prefetch_lines; i++) {
    	        ...
    	        // My Code:
    		   	...
    		   	UINT32 prefetch_setIndex;
    
		    	// Calculate the tag and set index for the prefetch address
		    	SplitAddress(prefetch_addr, L2LineShift(), L2SetIndexMask(), prefetch_tag, prefetch_setIndex);
		    	SET &prefetch_set = _l2_sets[prefetch_setIndex];

		    	// Check if the line is already in the cache
		    	if (!prefetch_set.Find(prefetch_tag)) {
			// If not present, prefetch it
			prefetch_set.Replace(prefetch_tag);
			//cout << "Prefetching into L2: " << prefetch_addr << endl;

			// Optionally, you can also prefetch into L1 cache if needed
			if (L1_PREFETCH_ENABLED) {
			    ADDRINT l1_prefetch_addr = prefetch_addr;
			    for (UINT32 j = 0; j < _l1_prefetch_lines; j++) {
				l1_prefetch_addr += L1BlockSize();
				CACHE_TAG l1_prefetch_tag;
				UINT32 l1_prefetch_setIndex;
				SplitAddress(l1_prefetch_addr, L1LineShift(), L1SetIndexMask(), l1_prefetch_tag, l1_prefetch_setIndex);
				SET &l1_prefetch_set = _l1_sets[l1_prefetch_setIndex];
				
				// Check if the line is already in the cache
				if (!l1_prefetch_set.Find(l1_prefetch_tag)) {
				    // If not present, prefetch it into L1 cache
				    l1_prefetch_set.Replace(l1_prefetch_tag);
				    // Print the prefetch address for L1 cache
                    		    //cout << "Prefetching into L1: " << l1_prefetch_addr << endl;
				}
			    }
        		}
    			}
